...
{
	AProjectZCharacter* ProjectZCharacter = Cast<AProjectZCharacter>(OtherActor);
	if (ProjectZCharacter)
	{
		ProjectZCharacter->SetOverlappingWeapon(this);
	}
}
...
void AWeapon::OnSphereEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	...
	if (ProjectZCharacter)
	{
		ProjectZCharacter->SetOverlappingWeapon(nullptr);
	}
}
...
{
	WeaponState = State;// onrep_weaponstate 호출
	switch (WeaponState)
	{
	case EWeaponState::EWS_Equipped:
		ShowPickupWidget(false);
...
void AWeapon::Fire(const FVector& HitTarget, const FVector& MuzzleStart)
{
	if (FireAnimation)
	{
...
{
	Ammo = FMath::Clamp(Ammo-AmmoToAdd,0,AmmoMaxCapacity);
	SetHUDAmmo();
}
